...
function LecturerMainTests() {
    ...
    const [lecturerTestDetails, setLecturerTestDetails] = useState([]);
    
    async function fetchData() {
        let response = await axios.get(baseUrl+"/classes/courses_with_quizzes/", config);
        console.log(response.data);
        setLecturerTestDetails(response.data);
    }
    useEffect(() => {
        fetchData();
    }, [])

    console.log(lecturerTestDetails)
  return (
    <div className='px-10 py-11 h-full'>
        <div className='min-h-screen w-full p-6 bg-white rounded-md flex flex-col gap-4'>
            <div className='flex flex-row justify-between'>
                ...
                <p className='font-bold text-custom-green-two'>Total: {lecturerTestDetails.length}</p>
            </div>
            <div className='flex flex-wrap justify-around gap-2'>
                {   
                    lecturerTestDetails?.map((test, id) => (
                        <Link key={id} to={'/lecturer/assignments/'+test.id} className='w-[230px] h-fit flex flex-col justify-between gap-4 py-6 px-5 pb-4 mb-5 box-border shadow-nav-shadow rounded-md group cursor-pointer hover:bg-custom-off-white ease-in-out duration-500 hover:-translate-y-3.5'>
...
export default LecturerMainTests