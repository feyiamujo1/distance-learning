...
function LecturersMainAssignments() {
    ...
    const [lecturerAssignmentDetails, setLecturerAssignmentDetails] = useState([]);
    ...
    console.log(typeof lecturerAssignmentDetails)
    ...
    <div className='px-10 py-11 h-full'>
        <div className='min-h-screen w-full p-6 bg-white rounded-md flex flex-col gap-4'>
            <div className='flex flex-row justify-between'>
                ...
                <p className='font-bold text-custom-green-two'>Total: {lecturerAssignmentDetails?.length}</p>
            </div>
            <div className='flex flex-wrap justify-around gap-2 min-h-screen'>
                {
                    lecturerAssignmentDetails?.map((assignment, id) => (
                        <Link key={id} to={'/lecturer/assignments/'+assignment.id} className='w-[230px] h-fit flex flex-col justify-between gap-4 py-6 px-5 pb-4 mb-5 box-border shadow-nav-shadow rounded-md group cursor-pointer hover:bg-custom-off-white ease-in-out duration-500 hover:-translate-y-3.5'>
